import { ReceiptData, SVGRenderOptions } from './types';
import { generateJaggedEdgePath, generateBarcodeBars } from './layout';

export function renderReceiptSVG(data: ReceiptData, options: SVGRenderOptions = {}): string {
  ...
  const teethCount = 28;
  const toothH = 5;
  const topEdge = generateJaggedEdgePath(paperX, paperY, paperWidth, teethCount, toothH, true);
  const bottomEdge = generateJaggedEdgePath(paperX, paperY + paperHeight, paperWidth, teethCount, toothH, false);
  ...
  // Barcode Section
  if (data.barcode) {
    const barWidths = generateBarcodeBars(data.barcode.code);
    const barcodeH = 34;
...
function renderDashedLine(x1: number, x2: number, y: number): string {
  return `<line x1="${x1}" y1="${y}" x2="${x2}" y2="${y}" stroke="#000000" stroke-opacity="0.2" stroke-width="1.2" stroke-dasharray="4,4" />`;
}

function renderSolidLine(x1: number, x2: number, y: number): string {
